...
void YOLOv8_obb::preprocessGPU(const cv::Mat& image)
{
...
void YOLOv8_obb::postprocessGPU(std::vector<Object>& objs, float score_thres, float iou_thres, int topk, int num_labels)
{
    auto num_channels = this->output_bindings[0].dims.d[1]; // 20
    auto num_anchors = this->output_bindings[0].dims.d[2];  // 21504
    ...
    cuda_postprocess(objs, d_output, num_channels, num_anchors, 
                        this->pparam, score_thres, iou_thres, topk, num_labels);
}
...
void YOLOv8_obb::postprocess(std::vector<Object>& objs, float score_thres, float iou_thres, int topk, int num_labels)
{
    ...
    cv::Mat output = cv::Mat(num_channels, num_anchors, CV_32F, static_cast<float*>(this->host_ptrs[0]));  // (20, 21504)
    output         = output.t();  // (21504, 6)
                              ...
                              const std::vector<std::vector<unsigned int>>& COLORS)
{
    ...
    for (auto& obj : objs) {
        ...
        char text[256];
        sprintf(text, "object: %.1f%%", obj.prob * 100);
